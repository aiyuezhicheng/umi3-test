...
type TableProps = {
  showDetailInfoDrawer: (action: string ,id?: string) => void;
  deleteOneAsset: (oneAssetID: string) => void;
  tableList: AssetTreeItem[];
...
const Table: React.FC<TableProps> = (props) => {
  ...
  // const [selectedRowsState, setSelectedRows] = useState<AssetTreeItem[]>([]);

  useEffect(() => {
    if (searchKeyWords) {
      const newTableList = tableList.filter((item: AssetTreeItem) =>
  ...
  const columns: ProColumns<AssetTreeItem>[] = [
    {
